...
from src.chronos.pipeline_stages import (
    invalid_pipeline_stage_message,
...
async def run_pipeline(stage: str = "full") -> str:
    ...
    - graph: Extract knowledge-graph data
    - reindex: Rebuild the vector index for all events
    - full: Run all stages sequentially
    - backfill: Run the full pipeline with a page-by-page Plaud history scan

    Args:
        stage: Pipeline stage to run. Aliases all_history and full_history
            normalize to backfill.
    """
    try:
        try:
            ...
            return json.dumps(
                {
                    "error": invalid_pipeline_stage_message(stage)
                }
            )
...
async def ask_chronos(question: str, reasoning: str = "none") -> str:
    ...
    """
    try:
        from src.chronos.ask_context import build_ask_context, build_sources_from_results
        ...
        sources = build_sources_from_results(results, limit=5)
        ask_service = ChronosAskService()
        result = ask_service.ask(question, context_events, reasoning=reasoning)
        if "error" in result:
        ...
        return json.dumps(
            {
                ...
                "provider": (result.get("config") or {}).get("provider"),
