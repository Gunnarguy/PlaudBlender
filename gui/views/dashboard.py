...
class DashboardView(BaseView):
    def _build(self):
        # Provider-aware nouns (Qdrant: collections + points; Pinecone: indexes + vectors)
        try:
        ...
        # Keep these nouns on the instance so later UI updates don't accidentally
        # revert to Pinecone terms.
        self._vector_noun = "Points" if _is_qdrant else "Vectors"
        self._index_noun = "Collection" if _is_qdrant else "Index"

        vector_noun = self._vector_noun
        index_noun = self._index_noun
        ...
        ttk.Label(
            hero,
            text="At-a-glance health of your Plaud workspace",
            style="Muted.TLabel",
        ).pack(anchor="w")
        ...
        self.health_badges = {
            ...
            "vector": ttk.Label(strip, text="Vector DB • —", style="Badge.TLabel"),
        ...
        self.cards = {
            ...
            "vector": StatCard(stats_frame, vector_noun, "🧭", 0),
        ...
        # Vector snapshot
        vector_box = ttk.LabelFrame(
            self, text="Vector DB Snapshot", padding=8, style="Panel.TLabelframe"
        )
        vector_box.pack(fill="x", pady=(10, 4))
        vector_box.columnconfigure((0, 1), weight=1)

        self.vector_labels = {
            "vectors": ttk.Label(
                vector_box, text=f"{vector_noun}: —", style="Muted.TLabel"
            ),
            "namespaces": ttk.Label(
                vector_box, text="Namespaces: —", style="Muted.TLabel"
            ),
            "dimension": ttk.Label(vector_box, text="Dim: —", style="Muted.TLabel"),
            "metric": ttk.Label(vector_box, text="Metric: —", style="Muted.TLabel"),
            "mismatch": ttk.Label(
                vector_box, text="", style="Muted.TLabel", foreground="#e67e22"
            ),
            "index": ttk.Label(
                vector_box, text=f"{index_noun}: —", style="Muted.TLabel"
            ),
            "provider": ttk.Label(vector_box, text="Provider: —", style="Muted.TLabel"),
            "namespace": ttk.Label(
                vector_box, text="Namespace: —", style="Muted.TLabel"
            ),
        }
        self.vector_labels["vectors"].grid(row=0, column=0, sticky="w", pady=2)
        self.vector_labels["namespaces"].grid(row=0, column=1, sticky="w", pady=2)
        self.vector_labels["dimension"].grid(row=1, column=0, sticky="w", pady=2)
        self.vector_labels["metric"].grid(row=1, column=1, sticky="w", pady=2)
        self.vector_labels["index"].grid(row=2, column=0, sticky="w", pady=2)
        self.vector_labels["provider"].grid(row=2, column=1, sticky="w", pady=2)
        self.vector_labels["namespace"].grid(row=3, column=0, sticky="w", pady=2)
        self.vector_labels["mismatch"].grid(
            row=4, column=0, columnspan=2, sticky="w", pady=2
        )

        # Index / namespace quick selector (bring control to dashboard)
        selector = ttk.Frame(vector_box, style="Main.TFrame")
        selector.grid(row=4, column=0, columnspan=2, sticky="ew", pady=(6, 0))
        selector.columnconfigure(1, weight=1)
        selector.columnconfigure(3, weight=1)

        ttk.Label(selector, text=index_noun, style="Muted.TLabel").grid(
            row=0, column=0, sticky="w"
        )
        self.index_var = tk.StringVar()
        self.index_combo = ttk.Combobox(
            selector, textvariable=self.index_var, state="readonly", width=24
        )
        ...
        ttk.Label(selector, text="Namespace", style="Muted.TLabel").grid(
            row=0, column=2, sticky="w"
        )
        self.namespace_var = tk.StringVar()
        self.namespace_combo = ttk.Combobox(
            selector, textvariable=self.namespace_var, state="readonly", width=18
        )
        self.namespace_combo.grid(row=0, column=3, sticky="ew", padx=(4, 0))
        self.namespace_combo.bind(
            "<<ComboboxSelected>>", lambda _: self._on_namespace_change()
        )

        self.refresh_btn = ttk.Button(
            selector,
            text="↻ Reload",
            command=lambda: self.call("load_pinecone_indexes"),
        )
        self.refresh_btn.grid(row=0, column=4, padx=(8, 0))

        # Pipeline status breakdown (raw -> processed -> indexed)
        pipeline = ttk.LabelFrame(
            self, text="Pipeline Status", padding=8, style="Panel.TLabelframe"
        )
        ...
        self.pipeline_progress = ttk.Progressbar(
            pipeline, orient=tk.HORIZONTAL, mode="determinate", maximum=100, value=0
        )
        self.pipeline_progress.grid(
            row=1, column=0, columnspan=3, sticky="ew", pady=(4, 0)
        )
        self.pipeline_progress_label = ttk.Label(
            pipeline, text="Waiting for data", style="Muted.TLabel"
        )
        self.pipeline_progress_label.grid(
            row=2, column=0, columnspan=3, sticky="w", pady=(2, 0)
        )

        # Notion Integration Panel
        notion_panel = ttk.LabelFrame(
            self, text="📓 Notion Integration", padding=8, style="Panel.TLabelframe"
        )
        ...
        self.notion_labels = {
            "status": ttk.Label(
                notion_panel, text="Status: Checking...", style="Muted.TLabel"
            ),
            "database": ttk.Label(
                notion_panel, text="Database: —", style="Muted.TLabel"
            ),
            "synced": ttk.Label(
                notion_panel, text="Pages Synced: —", style="Muted.TLabel"
            ),
            "last_sync": ttk.Label(
                notion_panel, text="Last Sync: Never", style="Muted.TLabel"
            ),
        }
        ...
        ttk.Button(
            notion_btns,
            text="📤 Push to Notion",
            command=lambda: self.call("notion_push"),
        ).pack(side="left", padx=2)
        ttk.Button(
            notion_btns,
            text="📥 Pull from Notion",
            command=lambda: self.call("notion_pull"),
        ).pack(side="left", padx=2)
        ttk.Button(
            notion_btns,
            text="🔄 Full Sync",
            command=lambda: self.call("notion_full_sync"),
        ).pack(side="left", padx=2)
        ttk.Button(
            notion_btns,
            text="🔍 Check Status",
            command=lambda: self.call("notion_check_status"),
        ).pack(side="left", padx=2)
        ttk.Button(
            notion_btns,
            text="⚙️ Configure",
            command=lambda: self.call("notion_configure"),
        ).pack(side="left", padx=2)

        # Recent recordings
        recent = ttk.LabelFrame(
            self, text="Recent Recordings", padding=8, style="Panel.TLabelframe"
        )
        recent.pack(fill="both", expand=True, pady=(4, 8))
        recent.columnconfigure(0, weight=1)
        recent.rowconfigure(1, weight=1)

        ttk.Label(recent, text="Latest sync pulls", style="Muted.TLabel").grid(
            row=0, column=0, sticky="w", pady=(0, 6)
        )

        cols = ("name", "date", "duration")
        self.recent_tree = ttk.Treeview(
            recent, columns=cols, show="headings", height=6, style="Treeview"
        )
        ...
        activity = ttk.LabelFrame(
            self, text="Activity", padding=8, style="Panel.TLabelframe"
        )
        activity.pack(fill="both", expand=True, pady=(0, 8))
        activity.columnconfigure(0, weight=1)
        ttk.Label(
            activity, text="Latest operations and status messages", style="Muted.TLabel"
        ).grid(row=0, column=0, sticky="w", pady=(0, 4))
        self.activity_list = tk.Text(
            activity,
            height=6,
            wrap=tk.NONE,
            state="disabled",
        ...
        scroll = ttk.Scrollbar(
            activity, orient=tk.VERTICAL, command=self.activity_list.yview
        )
        scroll.grid(row=1, column=1, sticky="ns")
        self.activity_list.configure(yscrollcommand=scroll.set)

        quick_actions = ttk.LabelFrame(
            self, text="Quick Actions", padding=8, style="Panel.TLabelframe"
        )
        quick_actions.pack(fill="x", pady=(12, 0))
        ttk.Label(
            quick_actions, text="Common flows in one click", style="Muted.TLabel"
        ...
        for idx, (text, action) in enumerate(buttons):
            ...
            ttk.Button(
                btn_row, text=text, style=style, command=lambda a=action: self.call(a)
        ...
        # One-shot refresh guard (so navigating away/back doesn't spam network calls)
        self._did_initial_refresh = False

    def on_show(self):  # pragma: no cover - UI
        """Refresh dashboard stats once when the dashboard is first shown.

        We intentionally defer heavy startup work to keep the GUI snappy.
        But the dashboard should still populate Vector DB Snapshot + selectors
        automatically (without loading all points).
        """
        ...
        # Both actions run their work in background threads.
        try:
            self.call("refresh_dashboard")
        except Exception:
            pass
        try:
            # Populate collections/namespaces + snapshot, but avoid bulk point listing.
            self.call("load_pinecone_indexes", False)
        except Exception:
            pass
    ...
    def update_stats(self, stats: dict):
        mapping = {
            ...
            "vector": ("vector", str),
            "vector_namespaces": ("namespaces", str),
        ...
        if any(
            k in stats for k in ("status_raw", "status_processed", "status_indexed")
        ):
        ...
        if any(
            ...
            for k in (
                ...
                "vector_namespaces",
                "vector_dim_mismatch",
                "pinecone_dim_mismatch",
            )
        ):
            mismatch = stats.get("vector_dim_mismatch") or stats.get(
                "pinecone_dim_mismatch"
            )
            vector_badge = self.health_badges["vector"]
            if mismatch:
                vector_badge.configure(
                    text="Vector DB • Needs attention", style="WarnBadge.TLabel"
                )
            else:
                vector_badge.configure(
                    text="Vector DB • Healthy", style="SuccessBadge.TLabel"
                )
        ...
        if any(
            ...
            self.update_vector_snapshot(
                {
                    "vectors": stats.get("vector") or stats.get("pinecone"),
                    "namespaces": stats.get("vector_namespaces")
                    or stats.get("pinecone_namespaces"),
                    "dimension": stats.get("vector_dim") or stats.get("pinecone_dim"),
                    "metric": stats.get("vector_metric")
                    or stats.get("pinecone_metric"),
                    "dim_mismatch": stats.get("vector_dim_mismatch")
                    or stats.get("pinecone_dim_mismatch"),
                    "index": stats.get("vector_index") or stats.get("pinecone_index"),
                    "provider": stats.get("vector_provider")
                    or stats.get("pinecone_provider"),
                    "namespace": stats.get("vector_namespace")
                    or stats.get("pinecone_namespace"),
                }
            )
        ...
        if stats.get("vector_dim_mismatch") or stats.get("pinecone_dim_mismatch"):
            self.next_steps.configure(
                text="⚠️ Dimension mismatch detected — open Vector tab and click Auto-fix."
            )
        elif stats.get("recordings", 0) == 0:
            self.next_steps.configure(
                text="📥 No recordings yet — click Fetch to pull Plaud transcripts."
            )
        else:
            self.next_steps.configure(
                text="✅ Everything looks good. Try Semantic Search or Sync All."
            )
    ...
    def update_recent_transcripts(self, transcripts):
        ...
        sorted_recs = sorted(
            transcripts,
            key=lambda r: (r.get("display_date", ""), r.get("display_time", "")),
            reverse=True,
        )
    ...
    def update_vector_snapshot(self, snapshot: dict):
        if not snapshot:
            return
        if snapshot.get("vectors") is not None:
            self.vector_labels["vectors"].configure(
                text=f"{getattr(self, '_vector_noun', 'Vectors')}: {snapshot.get('vectors')}"
            )
        if snapshot.get("namespaces") is not None:
            self.vector_labels["namespaces"].configure(
                text=f"Namespaces: {snapshot.get('namespaces')}"
            )
        if snapshot.get("dimension") is not None:
            self.vector_labels["dimension"].configure(
                text=f"Dim: {snapshot.get('dimension')}"
            )
        if snapshot.get("metric") is not None:
            self.vector_labels["metric"].configure(
                text=f"Metric: {snapshot.get('metric')}"
            )
        if snapshot.get("index") is not None:
            self.vector_labels["index"].configure(
                text=f"{getattr(self, '_index_noun', 'Index')}: {snapshot.get('index')}"
            )
        if snapshot.get("provider") is not None:
            self.vector_labels["provider"].configure(
                text=f"Provider: {snapshot.get('provider')}"
            )
        if snapshot.get("namespace") is not None:
            self.vector_labels["namespace"].configure(
                text=f"Namespace: {snapshot.get('namespace')}"
            )

        # Color-coded mismatch indicator
        if snapshot.get("dim_mismatch"):
            self.vector_labels["mismatch"].configure(
                text="🟡 Dimension mismatch — go to Vector tab and click Auto-fix",
                foreground="#e67e22",
            )
        else:
            self.vector_labels["mismatch"].configure(
                text="🟢 Index aligned with provider", foreground="#27ae60"
            )
        ...
        if counts and len(counts) == 3:
            ...
            if total:
                ...
                self.pipeline_progress_label.configure(
                    text=f"{completion}% indexed · {counts[2]} / {total}"
                )
            else:
    ...
    def update_notion_status(self, status: dict):
        ...
        if status.get("connected"):
            self.notion_labels["status"].configure(
                text="Status: 🟢 Connected", foreground="#27ae60"
            )
            ...
            self.notion_labels["status"].configure(
                text="Status: ⚪ Not configured", foreground="#7f8c8d"
            )
        ...
        if status.get("pages_synced") is not None:
            self.notion_labels["synced"].configure(
                text=f"Pages Synced: {status['pages_synced']}"
            )

        if status.get("last_sync"):
            self.notion_labels["last_sync"].configure(
                text=f"Last Sync: {status['last_sync']}"
            )
    ...
    def _update_env_badge(self):
        ...
        try:
            ...
            pkey = os.getenv("PINECONE_API_KEY") if provider == "pinecone" else True
            gem = os.getenv("GEMINI_API_KEY")
            openai_key = os.getenv("OPENAI_API_KEY") or os.getenv("OPENAI_APIKEY")

            missing_core = [
                name
                for name, val in {"Plaud ID": cid, "Plaud Secret": secret}.items()
                if not val
            ]
            if missing_core:
                self.health_badges["env"].configure(
                    text="Env • Missing keys", style="WarnBadge.TLabel"
                )
                return

            if provider == "pinecone" and not pkey:
                self.health_badges["env"].configure(
                    text="Env • Vector key missing", style="WarnBadge.TLabel"
                )
                return

            if not (gem or openai_key):
                self.health_badges["env"].configure(
                    text="Env • LLM key missing", style="WarnBadge.TLabel"
                )
                return

            self.health_badges["env"].configure(
                text="Env • Ready", style="SuccessBadge.TLabel"
            )
        except Exception:
            self.health_badges["env"].configure(
                text="Env • Check", style="WarnBadge.TLabel"
            )

    # ------------------------------------------------------------------
    # Vector selectors (legacy callback names retained for compatibility)
    # ------------------------------------------------------------------
