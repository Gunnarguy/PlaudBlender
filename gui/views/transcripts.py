...
class TranscriptsView(BaseView):
    def _build(self):
        ...
        _item_plural = "points" if _is_qdrant else "vectors"
        _container_term = "collection" if _is_qdrant else "index"
        ...
        ttk.Label(
            header, text="Browse and act on Plaud recordings", style="Muted.TLabel"
        ).pack(side=tk.LEFT, padx=(8, 0))

        filter_frame = ttk.Frame(header, style="Main.TFrame")
        filter_frame.pack(side=tk.RIGHT)

        ttk.Label(filter_frame, text="Filter:", style="Muted.TLabel").pack(
        ...
        filter_entry.bind(
            "<KeyRelease>",
            lambda _: self.call("filter_transcripts", self.filter_var.get()),
        )
        ttk.Button(
            filter_frame,
            text="Clear",
        ...
        toolbar = ttk.LabelFrame(
            self, text="Actions", padding=6, style="Panel.TLabelframe"
        )
        ...
        for text, action in actions:
            ...
            ToolTip(
                btn,
                {
                    ...
                    "delete_selected": f"Remove {_item_plural} for the selected recording from the active {_container_term}",
        ...
        table_frame = ttk.LabelFrame(
            self, text="Transcript List", padding=6, style="Panel.TLabelframe"
        )
        ...
        self.empty_label = ttk.Label(
            table_frame, text="No transcripts yet", style="Muted.TLabel"
        )
        ...
        detail = ttk.LabelFrame(
            self, text="Selection", padding=8, style="Panel.TLabelframe"
        )
        ...
        for row, (label, key) in enumerate(
            ...
            ttk.Label(detail, text=f"{label}:", style="Muted.TLabel").grid(
    ...
    def _apply_filters(self):
        ...
        filtered_records = [
            ...
            or rec.get("status", "raw") == self.status_filter
        ]
