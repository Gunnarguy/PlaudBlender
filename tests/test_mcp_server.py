...
class TestRunPipelineTool:
    ...
    @patch("scripts.mcp_server.subprocess.run")
    def test_backfill_alias_normalizes_to_canonical_stage(self, mock_run):
        from scripts.mcp_server import run_pipeline

        mock_run.return_value = MagicMock(
            returncode=0,
            stdout="Backfill complete.",
            stderr="",
        )
        ...
        parsed = json.loads(result)
        assert parsed["stage"] == "backfill"
        assert parsed["exit_code"] == 0
        assert mock_run.call_args.args[0][2] == "--backfill"
...
class TestAskChronosTool:
    ...
    @patch("scripts.mcp_server._get_data_service")
    def test_returns_provider_backed_answer(self, mock_get_ds):
        from scripts.mcp_server import ask_chronos

        mock_event = MagicMock()
        mock_event.id = "evt-1"
        mock_event.clean_text = "Reviewed the Ask Chronos flow."
        mock_event.start_ts = datetime(2026, 5, 18, 10, 0)
        mock_event.end_ts = datetime(2026, 5, 18, 10, 15)
        mock_event.category = "work"
        mock_event.recording_id = "rec-1"

        mock_result = MagicMock()
        mock_result.event = mock_event
        mock_result.score = 0.92
        mock_result.context_before = None
        mock_result.context_after = None

        mock_ds = MagicMock()
        mock_ds.search.return_value = [mock_result]
        mock_ds.get_events_for_recording.return_value = [mock_event]
        mock_ds.get_day_detail.return_value = None
        mock_get_ds.return_value = mock_ds

        with patch("src.chronos.ask_service.ChronosAskService") as MockAsk:
            ask_instance = MockAsk.return_value
            ask_instance.ask.return_value = {
                "answer": "You reviewed the Ask Chronos flow.",
                "model": "gpt-5.4",
        ...
        parsed = json.loads(result)
        assert parsed["answer"] == "You reviewed the Ask Chronos flow."
        assert parsed["provider"] == "openai"
        assert parsed["events_searched"] == 1
