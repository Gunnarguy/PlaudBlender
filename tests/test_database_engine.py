...
from src.database.engine import get_engine, init_db, get_db, _ensure_sqlite_additive_schema
...
def test_get_engine_pragmas_file(tmp_path):
    ...
    with engine.connect() as conn:
        ...
        assert journal_mode.upper() in ("WAL", "MEMORY", "DELETE") # Depending on env it may vary, but should not crash
