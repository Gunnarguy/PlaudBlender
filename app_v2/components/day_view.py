...
_RECORDING_ONLY_HEAT_COLOR = "#a16207"
...
def _summarize_zero_event_recordings(recordings: List[RecordingSummary]) -> str:
    """Describe why a recorded day has no extracted moments yet."""
    status_counts = Counter(
        str(getattr(recording, "processing_status", "completed") or "completed")
        .strip()
        .lower()
    ...
    labels = [
        ("failed", "sync failed", "failed syncs"),
        ("processing", "processing", "processing"),
        ("pending", "sync pending", "pending syncs"),
        ("completed", "moments not extracted", "recordings missing moments"),
    ]
    ...
    for status, singular, plural in labels:
        ...
        if count <= 0:
            continue
        parts.append(singular if count == 1 else f"{count} {plural}")
...
def create_recording_card(recording: RecordingSummary, day_date: str) -> html.Div:
    ...
    if proc_status in ("pending", "processing", "failed") and recording.event_count == 0:
        hour = recording.start_time.hour
        ambient = _time_of_day_label(hour)
        status_label, status_color, helper_text = {
            "pending": (
                "⏳ Waiting to process",
                "#f59e0b",
                "Run Full Sync to extract moments from this recording.",
            ),
            "processing": (
                "🔄 Processing…",
                "#3b82f6",
                "Chronos is currently extracting moments from this recording.",
            ),
            "failed": (
                "⚠️ Sync failed",
                "#ef4444",
                "Open Sync to retry extracting moments from this recording.",
            ),
        }[proc_status]
        ...
        system_strip = build_recording_system_strip(recording)
        return html.Div(
            id={"type": "recording-card", "id": recording.recording_id, "date": day_date},
            className=f"recording-card recording-cat-unknown recording-status-empty recording-{proc_status}",
            style={"borderLeft": f"3px solid {status_color}", "opacity": "0.8"},
            children=[
                html.Div(
    ...
    system_strip = build_recording_system_strip(recording)
    if system_strip is not None:
        children.append(system_strip)
...
def _build_day_flow_narrative(recordings: list) -> str:
    ...
    if not recordings:
        return ""
    sorted_recs = sorted(
        (recording for recording in recordings if recording.event_count > 0),
...
def create_day_card(day: DaySummary, expanded: bool = False) -> html.Div:
    ...
    if day.recordings:
        ...
        if cat_labels:
            ...
            status_note = _summarize_zero_event_recordings(day.recordings)
            day_summary_text = (
                f"{span_label}  •  {status_note}" if status_note else span_label
            )
    else:
...
def create_heat_map_strip(
    ...
    for offset in range(num_calendar_days - 1, -1, -1):
        ...
        recorded_without_moments = rec_count > 0 and count == 0
        color = (
            _RECORDING_ONLY_HEAT_COLOR
            if recorded_without_moments
            else _heat_color(count)
        )
        ...
        tooltip = f"{d.strftime('%b %-d')}: {count} moments, {rec_count} recordings"
        if recorded_without_moments and day_data:
            status_note = _summarize_zero_event_recordings(day_data.recordings)
            tooltip = (
                f"{d.strftime('%b %-d')}: {rec_count} recording"
                f"{'s' if rec_count != 1 else ''}, 0 moments"
                + (f" — {status_note}" if status_note else "")
            )

        classes = ["heatmap-cell"]
        if recorded_without_moments:
            classes.append("heatmap-recording-only")
        if is_today:
            classes.append("heatmap-today")
