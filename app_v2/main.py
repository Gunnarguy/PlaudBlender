...
_xray_route_cache = TTLCache()
_XRAY_COST_SESSION_TTL_SECONDS = 3.0
_XRAY_COST_HISTORY_TTL_SECONDS = 30.0
_XRAY_THROUGHPUT_TTL_SECONDS = 1.0
...
def _register_xray_routes(server):
    ...
    def xray_api_costs():
        ...
        days = max(int(request.args.get("days", 30, type=int) or 30), 1)
        session = _xray_route_cache.get_or_compute(
            ("xray-costs-session",),
            _XRAY_COST_SESSION_TTL_SECONDS,
            get_session_cost,
        )
        historical = _xray_route_cache.get_or_compute(
            ("xray-costs-history", days),
            _XRAY_COST_HISTORY_TTL_SECONDS,
    ...
    def xray_api_throughput():
        ...
        bucket_count = max(int(request.args.get("buckets", 30, type=int) or 30), 1)
        bucket_count = min(bucket_count, 60)
        buckets = _xray_route_cache.get_or_compute(
            ("xray-throughput", bucket_count),
            _XRAY_THROUGHPUT_TTL_SECONDS,
            lambda: get_throughput(bucket_count),
        )
