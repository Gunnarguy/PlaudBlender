...
(function () {
  ...
  function boot() {
    ...
    if (!pip) { setTimeout(boot, 200); return; }
    init(pip);
  }
  setTimeout(boot, 300);
  ...
  function init(pip) {
    ...
    document.getElementById('xp-filters').addEventListener('click', function (e) {
      ...
      if (!btn) return;
      pip.querySelectorAll('.xp-fbtn').forEach(function (b) { b.classList.remove('active'); });
    ...
    document.getElementById('xp-titlebar').addEventListener('dblclick', function () {
    ...
    (function () {
      ...
      var resizing = false, sx = 0, sy = 0, sw = 0, sh = 0, origRight = 0, origBottom = 0;
    ...
    // ── Rendering helpers ──
    function tier(ms) { return ms < 100 ? 'fast' : ms < 500 ? 'med' : 'slow'; }
    function barPct(ms) {
      if (ms <= 0) return 5;
      return Math.min(100, Math.max(5, (Math.log10(ms) / Math.log10(2000)) * 100)) | 0;
    }
    function fmtDur(ms) { return ms < 1000 ? ms.toFixed(0) + 'ms' : (ms / 1000).toFixed(1) + 's'; }
    function fmtTs(ts) {
    ...
    function filterEvents() {
      ...
      if (filter === 'errors') return allEvents.filter(function (e) { return e.level === 'error' || e.level === 'warn'; });
      var group = FILTER_GROUPS[filter];
      if (group) {
        return allEvents.filter(function (e) { return group.indexOf(e.source) !== -1; });
      }
      return allEvents.filter(function (e) { return e.source === filter; });
    }
    ...
    function buildRow(e) {
      ...
      var lvlCls = lvl === 'warn' ? ' warn' : lvl === 'error' ? ' error' : lvl === 'perf' ? ' perf' : '';

      var dur = '';
      if (e.duration_ms != null) {
        var t = tier(e.duration_ms), p = barPct(e.duration_ms);
        dur = '<span class="xp-dur"><span class="xp-bar"><span class="xp-fill ' + t + '" style="width:' + p + '%"></span></span><span class="xp-dlabel ' + t + '">' + fmtDur(e.duration_ms) + '</span></span>';
      }
      var dtl = e.detail ? '<span class="xp-dtl">' + esc(e.detail) + '</span>' : '';

      return '<div class="xp-row' + lvlCls + '">' +
        '<span class="xp-ts">' + fmtTs(e.ts) + '</span>' +
        '<span class="' + srcCls + '"><span style="margin-right:3px">' + meta.icon + '</span>' + esc(meta.label) + '</span>' +
        '<span class="xp-op">' + esc(opLabel(e.op || '')) + '</span>' +
        '<span class="xp-msg">' + esc(e.message || '') + '</span>' +
        dur + dtl + '</div>';
    }
    ...
    function renderAll() {
      ...
      if (!evts.length) {
        list.innerHTML = '<div class="xp-empty"><span class="xp-empty-icon">📡</span>No activity yet.</div>';
      } else {
    ...
    function updateStats() {
      ...
      var timed = allEvents.filter(function (e) { return e.duration_ms != null; }).map(function (e) { return e.duration_ms; });
      var avg = timed.length ? timed.reduce(function (a, b) { return a + b; }, 0) / timed.length : 0;
      var peak = timed.length ? Math.max.apply(null, timed) : 0;
      var errs = allEvents.filter(function (e) { return e.level === 'error'; }).length;
      ...
      if (timed.length) {
        h += '<span class="xp-stat">avg <b class="' + avgC + '">' + fmtDur(avg) + '</b></span>';
        h += '<span class="xp-stat">peak <b class="' + peakC + '">' + fmtDur(peak) + '</b></span>';
      }
      if (errs) h += '<span class="xp-stat"><b class="r">' + errs + '</b> err</span>';
      if (warns) h += '<span class="xp-stat"><b class="w">' + warns + '</b> warn</span>';
      stats.innerHTML = h;
    }
