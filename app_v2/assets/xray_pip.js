...
(function () {
  ...
  var POLL_INTERVAL_IDLE = 3000;
  var POLL_INTERVAL_HIDDEN = 5000;
  var THROUGHPUT_POLL_INTERVAL_ACTIVE = 4000;
  var THROUGHPUT_POLL_INTERVAL_HIDDEN = 10000;
  var COST_POLL_INTERVAL_ACTIVE = 15000;
  var COST_POLL_INTERVAL_HIDDEN = 30000;
  var SPARKLINE_BARS = 30;
  var expandedSeq = null; // which event row is expanded (seq or null)
  var eventPollTimer = null;
  var throughputPollTimer = null;
  var costPollTimer = null;
  var eventsPollInFlight = false;
  var throughputPollInFlight = false;
  var costPollInFlight = false;
  ...
  function init(pip) {
    ...
    function togglePause() {
      ...
      scheduleEventPoll(paused ? nextEventDelay() : 0);
      scheduleThroughputPoll(paused ? nextThroughputDelay() : 0);
      scheduleCostPoll(paused ? nextCostDelay() : 0);
    }
    ...
    function pollCosts() {
      if (paused) {
        scheduleCostPoll(nextCostDelay());
        return;
      }
      if (costPollInFlight) {
        scheduleCostPoll(nextCostDelay());
    ...
    function isPanelBackgrounded() {
      return document.hidden || pip.classList.contains("minimized");
    }

    function nextEventDelay() {
      if (isPanelBackgrounded()) return POLL_INTERVAL_HIDDEN;
      return highestSeq > 0 ? POLL_INTERVAL_ACTIVE : POLL_INTERVAL_IDLE;
    }

    function nextThroughputDelay() {
      return isPanelBackgrounded()
        ? THROUGHPUT_POLL_INTERVAL_HIDDEN
        : THROUGHPUT_POLL_INTERVAL_ACTIVE;
    }

    function nextCostDelay() {
      return isPanelBackgrounded()
        ? COST_POLL_INTERVAL_HIDDEN
        : COST_POLL_INTERVAL_ACTIVE;
    }

    function scheduleEventPoll(delay) {
      clearTimeout(eventPollTimer);
      eventPollTimer = setTimeout(pollEvents, Math.max(0, delay || 0));
    }

    function scheduleThroughputPoll(delay) {
      clearTimeout(throughputPollTimer);
      throughputPollTimer = setTimeout(pollThroughput, Math.max(0, delay || 0));
    }

    function scheduleCostPoll(delay) {
      clearTimeout(costPollTimer);
      costPollTimer = setTimeout(pollCosts, Math.max(0, delay || 0));
    }

    function pollEvents() {
      if (paused) {
        scheduleEventPoll(nextEventDelay());
        return;
      }
      if (eventsPollInFlight) {
        scheduleEventPoll(nextEventDelay());
        return;
      }
    ...
    function pollThroughput() {
      if (paused) {
        scheduleThroughputPoll(nextThroughputDelay());
        return;
      }
      if (throughputPollInFlight) {
        scheduleThroughputPoll(nextThroughputDelay());
        return;
      }
      throughputPollInFlight = true;
