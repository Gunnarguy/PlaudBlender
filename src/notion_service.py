...
class NotionServiceError(RuntimeError):
    """Base error for Notion service failures that should surface to callers."""


class NotionTimeoutError(NotionServiceError):
    """Raised when the Notion API stalls or times out."""


class NotionConnectionError(NotionServiceError):
    """Raised when the Notion API cannot be reached."""


class NotionAuthError(NotionServiceError):
...
class NotionService:
    ...
    @staticmethod
    def _classify_error(operation: str, exc: Exception) -> NotionServiceError:
        raw_message = str(exc).strip() or exc.__class__.__name__
        lowered = raw_message.lower()

        if any(token in lowered for token in ("timed out", "timeout")):
            return NotionTimeoutError(f"{operation} timed out: {raw_message}")
        if any(
            token in lowered
            for token in (
                "connection refused",
            ...
            return NotionConnectionError(f"{operation} failed: {raw_message}")
        if any(
            token in lowered
            for token in (
                "unauthorized",
            ...
            return NotionAuthError(f"{operation} failed: {raw_message}")
        return NotionServiceError(f"{operation} failed: {raw_message}")
    ...
    def fetch_recordings(
        ...
        try:
            ...
            classified = self._classify_error("Fetching Notion recordings", e)
            logger.error(str(classified))
            if raise_on_error:
                raise classified from e
            return []

    def fetch_page_content(self, page_id: str, *, raise_on_error: bool = False) -> str:
        ...
        try:
            ...
            classified = self._classify_error(
                f"Fetching Notion page content for {page_id}", e
            )
            logger.warning(str(classified))
            if raise_on_error:
                raise classified from e
            return ""
